 ...
 * resting state stays clean, no permanent "Click to edit"
 * instructional text fighting for attention with the actual size
 * value. Stroke-width bumped from the BaseIcon default 1.75 to 2
 * for this icon so the pencil tip and diagonal stroke read at the
 * 12px display size used in the tile's `trailing` slot.
 */
export function IconPencil({ size, ...p }: IconProps) {
  return (
    <svg
      width={size}
      height={size}
